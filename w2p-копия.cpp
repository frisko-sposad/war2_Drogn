...
void v_human1(bool rep_init)
{
    ...
    else
    {
        ...
        sort_stat(S_COLOR, P_RED, CMP_EQ); // выбираем владельца красным
        ...
        if (units > 0)
        {
            ...
            else
            {
                ...
                {
                    // тут вся хуйня что сделать после таймера
                    viz_area_add(59, 39, 1 << P_WHITE, 5); // открыть карту белого размером в 5 клеток
                    char message[] = "Кастелян: Беги на заставу, позвони в колокол на башне!";
