...
void bullet_disp_fires()
{
    ...
    while (k > 0)
    {
        ...
        if (f)
        {
            ...
            if ((life > 0) && ((life < 180) && ((life % 20) == 0)))
                ((void (*)(int *, int))0x00410520)(p, 1); // damage_area(bullet, hit_air); every 20 tiks
            if (life > 0)
                life--;
            char buf[] = "\x0";
            buf[0] = life;
        ...
        if (pflag & IS_FLESHY)
        {
            if ((id != U_PEASANT) && (id != U_PEON))
            {
                ...
                if (k > 0)
                {
                    ...
                    if ((a == ANIM_MOVE) || (a == ANIM_ATTACK))
                        set_stat(u, 255, S_ANIMATION_TIMER);
                }
            }
        }
    }
...
int grp_draw_bullet(int a, int *u, void *grp, int frame)
{
    ...
    if (id == B_BLIZZARD)
    {
        ...
        if (cid == U_MAGE)
            new_grp = grp_bliz;
        else if (cid == U_BALLISTA)
            new_grp = grp_ballistabliz;
    }
...
void set_yellow_palette()
{
    ...
    int k = 0xB0;
    set_rgb8((DWORD *)(SCREEN_INITIAL_PALETTE + k * 4), 4, 4, 0);
    k++;
    set_rgb8((DWORD *)(SCREEN_INITIAL_PALETTE + k * 4), 6, 6, 0);
    k++;
    set_rgb8((DWORD *)(SCREEN_INITIAL_PALETTE + k * 4), 8, 8, 0);
    k++;
    set_rgb8((DWORD *)(SCREEN_INITIAL_PALETTE + k * 4), 9, 9, 0);
    k++;
    set_rgb8((DWORD *)(SCREEN_INITIAL_PALETTE + k * 4), 11, 11, 0);
    k++;
    set_rgb8((DWORD *)(SCREEN_INITIAL_PALETTE + k * 4), 14, 14, 0);
    k++;
    set_rgb8((DWORD *)(SCREEN_INITIAL_PALETTE + k * 4), 15, 15, 0);
    k++;
    set_rgb8((DWORD *)(SCREEN_INITIAL_PALETTE + k * 4), 16, 16, 0);
    k++;
    set_rgb8((DWORD *)(SCREEN_INITIAL_PALETTE + k * 4), 18, 18, 0);
    k++;
...
{
    for (int i = 0; i < 18; i++)
        if (access(w_names[i], F_OK) != 0)
            w_sounds_e = false;
    for (int i = 0; i < 21; i++)
        if (access(hu_names[i], F_OK) != 0)
            hu_sounds_e = false;
    for (int i = 0; i < 12; i++)
        if (access(e_names[i], F_OK) != 0)
            e_sounds_e = false;
    for (int i = 0; i < 12; i++)
        if (access(k_names[i], F_OK) != 0)
            k_sounds_e = false;
    for (int i = 0; i < 12; i++)
        if (access(pk_names[i], F_OK) != 0)
            pk_sounds_e = false;
    for (int i = 0; i < 10; i++)
        if (access(wz_names[i], F_OK) != 0)
            wz_sounds_e = false;
    for (int i = 0; i < 11; i++)
        if (access(d_names[i], F_OK) != 0)
            d_sounds_e = false;
    for (int i = 0; i < 7; i++)
        if (access(g_names[i], F_OK) != 0)
            g_sounds_e = false;
    for (int i = 0; i < 10; i++)
        if (access(s_names[i], F_OK) != 0)
            s_sounds_e = false;
    if (access(hh1_name, F_OK) != 0)
        hh1_sounds_e = false;
    if (access(hh2_name, F_OK) != 0)
        hh2_sounds_e = false;
}
...
int empty_check_hero(byte id)
{
    ...
    if ((lvl == LVL_HUMAN3) || (lvl == LVL_XHUMAN3))
        m = false;
    if ((lvl == LVL_HUMAN6) || (lvl == LVL_XHUMAN6))
        m = false;
...
void buttons_init_paladin()
{
    ...
    {
        if (i < BUTTON_SIZE * 9)
            buttons_paladin[i] = *(byte *)(a + i);
        else
            buttons_paladin[i + BUTTON_SIZE] = *(byte *)(a + i);
    }
...
void magic_bliz(int *u)
{
    ...
    else if (caster == U_BALLISTA)
    {
        ...
        ((void (*)(int *))0x00442BE0)(u); // orig bliz
}
...
void magic_raise(int *u)
{
    ...
    if (caster == U_GRIFON)
    {
        ...
        ((void (*)(int *))0x00442D80)(u); // orig raise
    // set_stat(u, ORDER_STOP, S_NEXT_ORDER);
}
...
void magic_viz(int *u)
{
    ...
    if (caster == U_MAGE)
    {
        ...
        ((void (*)(int *))0x00442490)(u); // orig viz
}
...
int build_start_build(int *u, byte type, byte ordr)
{
    ...
    if (orig)
        return ((int (*)(int *, byte, byte))g_proc_00455F9C)(u, type, ordr); // original
    return original;
}
...
unsigned char *text_from_tutorial(int str_id)
{
    if (str_id == -1)
        ...
        return text_from_tbl(tbl_tutorial, str_id);
}
...
void v_human2(bool rep_init)
{
    ...
    {
        // your custom victory conditions
        if ((get_val(TH1, *(byte *)LOCAL_PLAYER) >= 1) && (get_val(FARM, *(byte *)LOCAL_PLAYER) >= 10))
            call_default_kill();
    }
}
...
void v_human4(bool rep_init)
{
    ...
    {
        // your custom victory conditions
        if (!slot_alive(*(byte *)LOCAL_PLAYER))
            lose(true);
        ...
        if (units >= 1)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                {
                    if (get_val(SHIPYARD, i) != 0)
                        f = false;
                    if (get_val(FOUNDRY, i) != 0)
                        f = false;
                    if (get_val(REFINERY, i) != 0)
                        f = false;
                    if (get_val(OIL_PLATFORM, i) != 0)
                        f = false;
                    if (get_val(TANKER, i) != 0)
                        f = false;
                    if (get_val(TRANSPORT, i) != 0)
                        f = false;
                    if (get_val(DESTROYER, i) != 0)
                        f = false;
                    if (get_val(BATTLESHIP, i) != 0)
                        f = false;
                    if (get_val(SUBMARINE, i) != 0)
                        f = false;
                }
            }
            if (f)
...
void v_human5(bool rep_init)
{
    ...
    else
    {
        ...
        if ((get_val(TH1, P_ORANGE) == 0) && (get_val(TH2, P_ORANGE) == 0) && (get_val(TH3, P_ORANGE) == 0) &&
            (get_val(BARRACKS, P_ORANGE) == 0) && (get_val(SHIPYARD, P_ORANGE) == 0))
            win(true);
    }
}
        ...
        *(byte *)(STARTING_CONTROLER_TYPE + P_VIOLET) = C_COMP;
        ...
        else
        {
            *(byte *)GB_HORSES = 0;
            ...
            if (*(byte *)(GB_HORSES + 1) > 50)
                unit_create(56, 28, U_DK, P_VIOLET, *(byte *)(GB_HORSES + 1) / 8);
            unit_create(56, 28, unc, P_VIOLET, *(byte *)(GB_HORSES + 1));
            if (*(byte *)(GB_HORSES + 1) > 30)
                unit_create(56, 28, U_GOBLINS, P_VIOLET, *(byte *)(GB_HORSES + 1) / 8);
            find_all_alive_units(ANY_MEN);
    ...
    else
    {
        ...
        if (units == 0)
        {
            if (*(byte *)(GB_HORSES + 5) == 0)
            {
                ...
                if (w_sounds_e)
                    sound_play_from_file(0, (DWORD)wr_name, wr_sound);
                else
                    gamesound_play(224, 12, 92);
                find_all_alive_units(U_PEASANT);
                sort_stat(S_OWNER, P_RED, CMP_EQ);
        ...
        else
        {
            ...
            if (*(byte *)GB_HORSES < 8)
                set_stat_all(S_PEON_TREE_CHOPS, 1);
        }
        find_all_alive_units(U_DANATH);
        if (units == 0)
        {
        ...
        else if (*(byte *)GB_HORSES == 6)
        {
            ...
            else
            {
                ...
                sort_stat(S_OWNER, P_WHITE, CMP_EQ);
                if (units < 20)
                    unit_create(74, 57, U_TROLL, P_WHITE, 1);
            }
        }
        ...
        else if (*(byte *)GB_HORSES == 13)
        {
            ...
            sort_stat(S_OWNER, P_RED, CMP_EQ);
            if (units == 0)
                unit_create(55, 9, U_MAGE, P_RED, 1);
            find_all_alive_units(U_MAGE_TOWER);
            sort_stat(S_OWNER, P_RED, CMP_EQ);
            set_stat_all(S_HP, 2000);
            find_all_alive_units(ANY_UNITS);
        ...
        else if (*(byte *)GB_HORSES == 15)
        {
            ...
            if (units != 0)
            {
                ...
                sort_stat(S_OWNER, P_RED, CMP_EQ);
                if (units != 0)
                    ((void (*)(int *, byte))F_SPELL_SOUND_UNIT)(unit[0], SS_THUNDER);
                remove_all();
            }
            else
                set_res(P_RED, 2, 10, 0, 0, 0);
...
int new_game(int a, int b, long c)
{
    ...
    game_started = true;
    if ((*(byte *)LEVEL_OBJ == LVL_HUMAN1) || (*(byte *)LEVEL_OBJ == LVL_XHUMAN1) || (*(byte *)LEVEL_OBJ == LVL_HUMAN10) || (*(byte *)LEVEL_OBJ == LVL_ORC1))
...
void draw_tips()
{
    ...
    if (k > 0)
    {
        ...
        {
            if (tip_line1 != NULL)
                draw_text(m_minx + 2 + 3, m_miny + 2 + 3 + 9 * 0, 251, tip_line1, 239, 0, false);
            if (tip_line2 != NULL)
                draw_text(m_minx + 2 + 3, m_miny + 2 + 3 + 9 * 1, 251, tip_line2, 239, 0, false);
            if (tip_line3 != NULL)
                draw_text(m_minx + 2 + 3, m_miny + 2 + 3 + 9 * 2, 251, tip_line3, 239, 0, false);
            if (tip_line4 != NULL)
                draw_text(m_minx + 2 + 3, m_miny + 2 + 3 + 9 * 3, 251, tip_line4, 239, 0, false);
        }
        if (tip_status == 3)
        {
            if (tip_w > 0)
        ...
        if (raindrops[i].l != 0)
        {
            if ((raindrops[i].x1 >= m_minx) && (raindrops[i].y1 >= m_miny) && (raindrops[i].x2 <= m_maxx) && (raindrops[i].y2 <= m_maxy))
            {
                if (raindrops_snow)
                {
                    ...
                    draw_line(raindrops[i].x1, raindrops[i].y1, raindrops[i].x2, raindrops[i].y2, RAINDROPS_COLOR, false);
            }
        }
    }
...
void drawing()
{
    ...
    if (*(byte *)GAME_MODE == 3)
    {
        if (can_rain)
            draw_rain();
        if (game_started)
            if ((*(byte *)LEVEL_OBJ == LVL_ORC1) || (*(byte *)LEVEL_OBJ == LVL_HUMAN10))
                draw_tips();
    }
